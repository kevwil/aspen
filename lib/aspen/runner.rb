...
module Aspen
  ...
  class Runner
    ...
    def self.commands
      commands  = COMMANDS
      commands += LINUX_ONLY_COMMANDS if Aspen.linux?
      commands
    end
    ...
    def initialize(argv)
      ...
      @options = {
        ...
        :log                  => File.join(Dir.pwd, 'log/aspen.log'),
    ...
    def parser
      ...
      # see <tt>lib/aspen/command.rb</tt>.
      @parser ||= OptionParser.new do |opts|
        ...
        unless Aspen.win? # Daemonizing not supported on Windows
          opts.separator ""
          ...
          opts.on(      "--all [DIR]", "Send command to each config files in DIR")      { |dir| @options[:all] = dir } if Aspen.linux?
        ...
        opts.on(      "--max-conns NUM", "Maximum number of open file descriptors " +
                                         ...
                                         "Might require sudo to set higher than 1024")  { |num| @options[:max_conns] = num.to_i } unless Aspen.win?
        ...
        opts.on(      "--no-epoll", "Disable the use of epoll")                         { @options[:no_epoll] = true } if Aspen.linux?
