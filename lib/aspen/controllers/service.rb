...
module Aspen
  module Controllers
    ...
    class Service < Controller
      INITD_PATH          = File.directory?('/etc/rc.d') ? '/etc/rc.d/aspen' : '/etc/init.d/aspen'
      DEFAULT_CONFIG_PATH = '/etc/aspen'
      TEMPLATE            = File.dirname(__FILE__) + '/service.sh.erb'
    
      def initialize(options)
        super
      
        raise PlatformNotSupported, 'Running as a service only supported on Linux' unless Aspen.linux?
      end
      ...
      def install(config_files_path=DEFAULT_CONFIG_PATH)
        if File.exist?(INITD_PATH)
          log_info "aspen service already installed at #{INITD_PATH}"
        else
          log_info "Installing aspen service at #{INITD_PATH} ..."
        ...
        log_info ''
        log_info "To configure aspen to start at system boot:"
