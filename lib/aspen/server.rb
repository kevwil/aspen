module Aspen
  ...
  #
  #   Aspen::Server.start('0.0.0.0', 3000, app)
  #
  # == UNIX domain server
  # Create a new UNIX domain socket bound to +socket+ file by specifiying a filename
  # as the first argument. Eg.: /tmp/aspen.sock. If the first argument contains a <tt>/</tt>
  # it will be assumed to be a UNIX socket. 
  #
  #   Aspen::Server.start('/tmp/aspen.sock', app)
  #
  ...
  #
  #   Aspen::Server.start('galaxy://faraway', 1345, app, :backend => Aspen::Backends::MyFancyBackend)
  #
  ...
  # 
  #   Aspen::Server.start('0.0.0.0', 3000) do
  ...
  class Server
    ...
    def start
      ...
      log_info  "Aspen web server (v#{VERSION::STRING} codename #{VERSION::CODENAME})"
    ...
    def stop!
      if @backend.started_reactor?
        ...
        log_info "Stopping Aspen ..."
        log_info "Aspen was started inside an existing EventMachine.run block."
    ...
    # This is also the name of the process in which Aspen is running as a daemon.
    def name
      "aspen server (#{@backend})" + (tag ? " [#{tag}]" : "")
    end
    alias :to_s :name
